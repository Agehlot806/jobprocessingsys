...
from account.models import User,MenuGroup,MenuMaster,TaskMaster,UserTaskAccess
...
class UserModelAdmin(BaseUserModelAdmin):
    ...
    list_display = ('id', 'email', 'usr', 'username', 'is_admin')
    list_filter = ('is_admin',)
    fieldsets = (
        ('User Credentials', {'fields': ('email', 'password')}),
        ('Personal info', {'fields': ('usr','username')}),
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('email', 'usr', 'username', 'password1', 'password2'),
        }),
    )
    search_fields = ('email',)
    ordering = ('email','id')
